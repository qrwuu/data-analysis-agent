...
import { renderSourceList, setSrc } from "./datasource.js";
...
function formatFileNameAsTitle(fileName = "") {
  ...
  if (lower.includes("category") || stem.includes("品类"))
    return "品类销售表现分析";
  if (lower.includes("channel") || stem.includes("渠道"))
    return "渠道销售表现分析";
  if (lower.includes("traffic") || stem.includes("流量"))
    return "店铺流量表现分析";
  if (
...
function summarizeUserQuestionAsTitle(question = "") {
  ...
  if (q.includes("SKU") || q.toLowerCase().includes("sku"))
    return "SKU销售表现分析";
  ...
  if (q.includes("画图") || q.includes("图表") || q.includes("可视化"))
    return "数据可视化分析";
  if (q.includes("销售")) return "销售表现分析";
  return q.length > 16 ? `${q.slice(0, 14)}分析` : `${q}分析`;
}

function _generateConversationTitle(conversation) {
  const direct =
  ...
  const savedAt = conversation?.saved_at
    ? conversation.saved_at.slice(0, 16).replace("T", " ")
    : "";
  const generatedName =
    !rawName ||
...
async function saveSession() {
  ...
  const r = await window.BAA.auth.authFetch(
    `/api/history/import-session/${encodeURIComponent(state.SID)}`,
    {
      method: "POST",
  ...
  closeOverlay("ov-save");
  toast("已保存到历史分析", "ok");
  window.BAA.sidebar?.setSessionName?.(
    name || d.session?.title || "当前分析",
    "",
  );
  await loadSavedList();
}
...
async function loadSavedList() {
  ...
  if (!window.BAA.auth?.isLoggedIn?.()) {
    box.innerHTML =
      '<div class="saved-empty">登录后可保存并查看你的历史分析记录。</div>';
    return;
  }
  ...
  if (!list.length) {
    box.innerHTML =
      '<div class="saved-empty">暂无历史分析，上传数据后开始你的第一次分析。</div>';
    return;
  }
  box.innerHTML = list
    .map((s) => {
...
async function loadHistorySession(historyId) {
  ...
  const r = await window.BAA.auth.authFetch(
    `/api/history/sessions/${encodeURIComponent(historyId)}`,
  );
  const data = await r.json().catch(() => ({}));
  if (!r.ok || !data.session) {
    toast(data.error || "历史分析加载失败", "err");
    return;
  }
  const restore = await window.BAA.auth.authFetch(
    `/api/history/sessions/${encodeURIComponent(historyId)}/restore/${encodeURIComponent(state.SID)}`,
    { method: "POST" },
  );
  if (!restore.ok) {
    toast("恢复历史分析失败", "err");
  ...
  for (const msg of data.messages || []) {
    if (msg.role === "user") appendMsg("user", msg.content);
      ...
      for (const cid of msg.chart_ids || [])
        bubble.before(window.BAA.chatStream.buildChartFrame(cid));
    }
  }
...
function hideLoadMask(timer) {
  if (
    activeLoad &&
    activeLoad.loadingId &&
    window.BAA.ui &&
    window.BAA.ui.isVue &&
    typeof window.BAA.ui.hideLoading === "function"
  ) {
...
async function loadSavedSession(filename, name) {
  if (
    !(await window.BAA.ui?.confirm?.({
      title: t("confirm.title"),
      message: t("confirm.load", { name }),
    }))
  )
    return;
  if (activeLoad) {
  ...
  try {
    ...
    for (const msg of d.history) {
      if (msg.role === "user") {
        ...
        if (msg.reasoning) {
          bubble.before(
            window.BAA.chatStream.buildReasoningBlock(msg.reasoning),
          );
        }
        for (const cid of msg.chart_ids || []) {
...
async function submitRenameSession() {
  ...
  const r = await fetch(
    `/api/saved-sessions/${encodeURIComponent(renameTarget.filename)}/rename`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function confirmDeleteSavedSession() {
  ...
  try {
    const r = await fetch(
      `/api/saved-sessions/${encodeURIComponent(filename)}`,
      { method: "DELETE" },
    );
