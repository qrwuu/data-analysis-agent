...
export function mountKnowledgeUi() {
  ...
  function _renderForm() {
    ...
    if (type === "metrics") {
      fieldNodes = [
        _renderField("指标名称", true, "text", "name", fields.name, v => { fields.name = v; }, "例如：DAU"),
        _renderField("别名（逗号分隔）", false, "text", "alias", fields.alias, v => { fields.alias = v; }, "日活, 日活跃用户"),
        _renderField("业务定义", false, "textarea", "definition", fields.definition, v => { fields.definition = v; }, "当日启动游戏一次及以上的独立设备数", 2),
        _renderField("SQL 模板", false, "textarea", "sql_template", fields.sql_template, v => { fields.sql_template = v; }, "SELECT COUNT(DISTINCT device_id) FROM events WHERE date='{date}'", 3),
        _renderField("口径备注", false, "textarea", "notes", fields.notes, v => { fields.notes = v; }, "剔除机器人流量；iOS/Android 分开统计", 2),
      ];
    } else if (type === "rules") {
      fieldNodes = [
        _renderField("规则 ID", true, "text", "rule_id", fields.rule_id, v => { fields.rule_id = v; }, "例如：retention_sanity"),
        _renderField("描述", false, "text", "description", fields.description, v => { fields.description = v; }, "次日留存不能超过首日 DAU"),
        _renderField("违反条件", false, "textarea", "condition", fields.condition, v => { fields.condition = v; }, "day2_retention > day1_dau", 2),
        _renderSelect("严重程度", "severity", fields.severity, v => { fields.severity = v; }, [
      ...
      // notes
      fieldNodes = [
        _renderField("主题", true, "text", "topic", fields.topic, v => { fields.topic = v; }, "例如：流失分析"),
        _renderField("内容", false, "textarea", "content", fields.content, v => { fields.content = v; }, "分析流失时需检查：版本更新、服务器波动、竞品上线…", 4),
        _renderField("标签（逗号分隔）", false, "text", "tags", fields.tags, v => { fields.tags = v; }, "流失, churn, 留存"),
      ];
    }
  ...
  function _renderField(label, required, inputType, fieldKey, value, onInput, placeholder, rows) {
    const labelNode = required
      ? h("label", null, [label, " ", h("span", { style: "color:#ef4444" }, "*")])
      : h("label", null, label);
    const attrs = { "data-kb-field": fieldKey, value, placeholder, onInput: e => onInput(e.target.value) };
    const inputNode = inputType === "textarea"
      ? h("textarea", { ...attrs, rows: rows || 2 })
      : h("input", { ...attrs, type: inputType });
    return h("div", { class: "f-row" }, [labelNode, inputNode]);
  }

  function _renderSelect(label, fieldKey, value, onChange, options) {
    return h("div", { class: "f-row" }, [
      h("label", null, label),
      h("select", { "data-kb-field": fieldKey, value, onChange: e => onChange(e.target.value) },
  ...
  function getFormValues() {
    const f = state.form;
    const fields = f.fields;
    root5.querySelectorAll("[data-kb-field]").forEach(input => {
      fields[input.dataset.kbField] = input.value;
    });
    if (f.type === "metrics") {
      return {
