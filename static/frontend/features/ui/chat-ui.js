...
export function mountChatUi() {
  ...
  function _renderAuditPanel(item) {
    ...
    const hasIssue = item.ok === false;
    if (hasIssue) classes.push("tool-audit-notice");
    const detail = hasIssue
      ? [item.status, item.content].filter(Boolean).join("\n\n")
      : item.content;
    if (detail) classes.push("tool-audit-has-summary");
    const attrs = {
      key: item.id,
      class: classes.join(" "),
      "data-tool": item.tool || "",
    ...
    const statusText = hasIssue ? "分析步骤已自动调整，点击查看详情" : item.status;
    const status = h(detail ? "summary" : "span", { class: "tool-audit-status" }, statusText);
    const body = detail
      ? h("div", { class: "tool-audit-summary" }, detail)
      : null;
    if (detail) {
      return h("details", {
        ...attrs,
  ...
  function _buildResultMeta(msg) {
    ...
    // A narrative response already has its own message bubble. Adding a
    // generated summary would duplicate the same text above it.
    if (!charts.length && !hasTable) return null;
  ...
  function _renderToolsFor(msg) {
    ...
    if (latestError) {
      statusTone = activeActivity || activeStep ? "running" : "done";
      statusText = activeActivity || activeStep
        ? "正在自动调整分析步骤"
        : "分析完成，部分步骤已自动调整";
