...
import { loadSavedList } from "../legacy/sessions.js";
  ...
  function _registerCommandHandlers() {
    ...
    commandHandlers.register("memory", () => {
      if (window.BAA.auth?.isLoggedIn?.()) window.openOverlay?.("ov-knowledge");
      else window.BAA.auth?.showKnowledgeGate?.();
    });
  ...
  async function _streamChat(payload, stepsEl, bubbleEl, typing) {
    ...
    try {
      const resp = await fetch(`/api/session/${state.SID}/chat`, {
        method: "POST", headers: window.BAA.auth?.authHeaders?.({ "Content-Type": "application/json" }) || { "Content-Type": "application/json" },
  ...
  function _onToolAudit(ev, ctx) {
    ...
    const tool = ev.tool || "";
    if (!tool) return;
    const hasIssue = ev.ok === false;
    const content = ev.content ?? ev.data ?? ev.summary;
    const panel = document.createElement(hasIssue || content ? "details" : "div");
    panel.className = hasIssue ? "tool-audit tool-audit-notice" : "tool-audit";
    ...
    statusLine.textContent = hasIssue ? "分析步骤已自动调整，点击查看详情" : (bits.length ? bits.join(" · ") : "工具执行完成");
    panel.appendChild(statusLine);
    const detail = hasIssue ? [bits.join(" · "), content].filter(Boolean).join("\n\n") : content;
    if (detail) {
  ...
  function _chartFrameNode(iframe) {
    return iframe?.closest?.(".chart-frame") || null;
  }

  function _chartStatusMessage(state) {
    if (state === "empty") return "暂无可视化数据，请重新生成图表";
    if (state === "error") return "图表渲染失败，请重新生成";
    return "图表加载中…";
  }

  function _setChartStatus(frame, state = "loading", message = "") {
    const status = frame?.querySelector?.(".chart-frame-status");
    const text = status?.querySelector?.(".chart-frame-status-text");
    if (!status || !text) return;
    status.dataset.state = state;
    text.textContent = message || _chartStatusMessage(state);
    status.classList.toggle("is-visible", state !== "ready");
  }

  function _normalizeChartDoc(doc) {
    if (!doc?.body) return;
    doc.body.style.background = "#fff";
    doc.body.style.margin = "0";
    doc.querySelectorAll(".plotly-graph-div, .plot-container").forEach(plot => {
      plot.style.width = "100%";
      plot.style.minHeight = "420px";
      if (!plot.style.height || plot.style.height === "100%" || plot.style.height === "0px") {
        plot.style.height = "420px";
      }
    });
  }

  function _graphHasData(plot) {
    try {
      const data = plot && (plot.data || plot._fullData || []);
      if (!Array.isArray(data) || !data.length) return false;
      return data.some(trace => {
        if (!trace) return false;
        return ["x", "y", "z", "labels", "values", "ids", "parents", "lon", "lat"].some(key => {
  ...
  function _graphHasVisuals(plot) {
  ...
  function _syncChartFrameHeight(iframe) {
    const frame = _chartFrameNode(iframe);
    try {
      const doc = iframe.contentDocument;
      if (!doc?.body) {
        _setChartStatus(frame, "loading", "");
        return false;
      }
      _normalizeChartDoc(doc);
      const graph = _chartGraph(iframe);
      const serverState = doc.documentElement?.dataset?.baaChartStatus || "";
      const serverMessage = doc.documentElement?.dataset?.baaChartMessage || "";

      if (!graph) {
        if (serverState === "empty") _setChartStatus(frame, "empty", serverMessage);
        else if (serverState === "error") _setChartStatus(frame, "error", serverMessage);
        else _setChartStatus(frame, "loading", "");
        iframe.style.height = "420px";
        return false;
      }

      if (graph.plotly?.Plots?.resize && graph.plot.classList.contains("js-plotly-plot")) {
        graph.plotly.Plots.resize(graph.plot);
      }

      const contentHeight = Math.max(
        doc.body.scrollHeight,
        doc.documentElement?.scrollHeight || 0,
        Math.round(graph.plot.getBoundingClientRect().height || 0) + 24,
        420,
      );
      iframe.style.height = contentHeight + "px";

      if (!_graphHasData(graph.plot) || serverState === "empty") {
        _setChartStatus(frame, "empty", serverMessage || _chartStatusMessage("empty"));
        return false;
      }
      if (_graphHasVisuals(graph.plot) || serverState === "ready") {
        _setChartStatus(frame, "ready", "");
        return true;
      }
      if (serverState === "error") {
        _setChartStatus(frame, "error", serverMessage || _chartStatusMessage("error"));
        return false;
      }
      _setChartStatus(frame, "loading", "");
      return false;
    } catch (_) {
      _setChartStatus(frame, "error", _chartStatusMessage("error"));
      iframe.style.height = "420px";
      return false;
    }
  }
  ...
  function _chartGraph(iframe) {
    ...
    if (!plot) return null;
    return { plot, plotly: win?.Plotly, doc, win };
  }

  async function _chartExportPayload(iframe) {
    const frame = _chartFrameNode(iframe);
    const graph = _chartGraph(iframe);
    if (!graph?.plot || !graph.plotly?.toImage) {
      _setChartStatus(frame, "error", _chartStatusMessage("error"));
      throw new Error("chart_not_ready");
    }
    if (!_graphHasData(graph.plot)) {
      _setChartStatus(frame, "empty", _chartStatusMessage("empty"));
      throw new Error("chart_empty");
    }
    _normalizeChartDoc(graph.doc);
    try {
      if (graph.plotly.relayout) {
        await graph.plotly.relayout(graph.plot, {
    ...
    const fullLayout = graph.plot._fullLayout || {};
    return {
      graph,
      width: Math.max(900, Math.round(fullLayout.width || rect.width || iframe.clientWidth || 900)),
      height: Math.max(520, Math.round(fullLayout.height || rect.height || iframe.clientHeight || 520)),
    };
  }

  async function _chartToImage(iframe) {
    const payload = await _chartExportPayload(iframe);
    return payload.graph.plotly.toImage(payload.graph.plot, {
      format: "png",
      width: payload.width,
  ...
  const _CHART_LOADING_ICON = _chartIcon([
    "M21 12a9 9 0 1 1-6.22-8.56",
  ]);

  function _setChartActionBusy(button, busy) {
    if (!button) return;
    ...
    if (busy) button.innerHTML = _CHART_LOADING_ICON;
    else if (button.dataset.originalHtml) button.innerHTML = button.dataset.originalHtml;
  }

  async function _runChartAction(button, action) {
    if (!button || button.disabled) return;
    _setChartActionBusy(button, true);
    try {
      await action();
    } finally {
      _setChartActionBusy(button, false);
    }
  }

  async function _triggerChartDownload(dataUrl, filename) {
    ...
    if (!response.ok || !result.ok || !result.download_url) {
      throw new Error("chart_export_failed");
    }
  ...
  async function _downloadChartImage(iframe, meta, options = {}) {
    try {
      const filename = _chartFileName(meta.title, meta.chartType);
      const dataUrl = await _chartToImage(iframe);
      await _triggerChartDownload(dataUrl, filename);
      _setChartStatus(_chartFrameNode(iframe), "ready", "");
      if (!options.silentToast) _chartToast("图片已下载", "ok");
      return true;
    } catch (_) {
      if (!options.silentToast) _chartToast("图片下载失败，请稍后重试", "err");
      return false;
    }
  }

  async function _copyChartImage(iframe, meta) {
    try {
      const dataUrl = await _chartToImage(iframe);
      const blob = _dataUrlToBlob(dataUrl);
      if (!navigator.clipboard?.write || typeof ClipboardItem === "undefined") {
        const downloaded = await _downloadChartImage(iframe, meta, { silentToast: true });
        _chartToast(
          downloaded ? "当前浏览器不支持直接复制图片，已为你下载图片" : "图片导出失败，请稍后重试",
          downloaded ? "ok" : "err",
        );
        return;
      }
  ...
  function _buildChartActions(iframe, chartTitle, chartType) {
    ...
    const meta = { title: chartTitle, chartType };
    ...
    copyBtn.innerHTML = _chartIcon([
      "M9 9h10v10H9z",
      "M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1",
    ]);
    copyBtn.addEventListener("click", () => _runChartAction(copyBtn, () => _copyChartImage(iframe, meta)));
    ...
    downloadBtn.innerHTML = _chartIcon([
      "M12 3v12",
      "M7 10l5 5 5-5",
      "M5 21h14",
    ]);
    downloadBtn.addEventListener("click", () => _runChartAction(downloadBtn, () => _downloadChartImage(iframe, meta)));
  ...
  function _buildChartFrame(chartId, chartTitle = "", chartType = "") {
    ...
    stage.className = "chart-frame-canvas";
    ...
    const sync = () => _syncChartFrameHeight(iframe);
    ...
    iframe.addEventListener("error", () => {
      _setChartStatus(_chartFrameNode(iframe), "error", _chartStatusMessage("error"));
    });

    const status = document.createElement("div");
    status.className = "chart-frame-status is-visible";
    status.dataset.state = "loading";
    status.innerHTML = '<div class="chart-frame-status-text">图表加载中…</div>';
  ...
  function _outlineMeta(ev) {
    ...
    if (ev.type === "ppt_outline") {
      ...
      headerTitle = esc(ev.filename || "数据导出");
      confirmPayload = { excel_tables: ev.tables, excel_filename: ev.filename, excel_format: ev.format || "xlsx", excel_sql: ev.sql || "", excel_row_limit: ev.row_limit || 0 };
