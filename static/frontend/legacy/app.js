...
import * as auth from "./auth.js?v=preference-memory-1";
...
(function () {
  ...
  function openSidebarDrawer(tab = "sessions") {
    if (tab === "sessions" && !window.BAA.auth?.isLoggedIn?.()) {
  ...
  const ACTIONS = {
    ...
    loadHistorySession: (_el, historyId) => sessions.loadHistorySession(historyId),
    ...
    openKnowledge:  () => {
      if (!auth.isLoggedIn()) { auth.showKnowledgeGate(); return; }
    ...
    removeSelectedXlFile: (_el, index, event) => datasource.removeSelectedXlFile(index, event),
  ...
  (async () => {
    ...
    // Remove the legacy persistent runtime key. Formal history is loaded only
    // from the authenticated, user-scoped history API.
    ...
    const safeInitStep = async (label, run, timeoutMs = 6000) => {
      ...
      try {
        ...
        window.__BAA_BOOT_GUARD?.report?.("init", `init-step-failed:${label}`, { stage: label, stack: error?.stack || String(error) });
    ...
    await safeInitStep("commands-and-skills", () => Promise.all([
    ...
    await safeInitStep("builtin-providers", () => window.BAA.models.loadBuiltinProviders());
    ...
    await safeInitStep("datasource-configs", () => datasource.loadDatasourceConfigs());
    ...
    if (window.BAA.workspace) {
      await safeInitStep("workspace-status", () => window.BAA.workspace.loadStatus());
    }

    // Check for a resumable auto-save from the previous session
    await safeInitStep("autosave-restore", () => autosave.checkAutosaveOnLoad());
