...
import { $, hideWelcome } from "../core/dom.js";
  ...
  async function triggerAutosave() {
    // Formal history is stored in the authenticated SQLite history service.
    // Guests remain temporary and must never create disk-backed chat archives.
    return;
    if (!state.SID) return;
    try {
      const body = {};
  ...
  async function checkAutosaveOnLoad() {
    // Legacy JSON autosaves are deliberately disabled. Guest conversations are
    // page-temporary; authenticated turns are persisted by the history service.
    return;
