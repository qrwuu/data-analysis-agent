...
import { invalidate as invalidatePreview, openSchemaView } from "./preview.js";
  ...
  function validateXlFiles(files) {
    ...
    for (const file of files || []) {
      ...
      if ((file?.size || 0) > XL_MAX_BYTES) return "文件过大，请上传 20MB 以内的数据文件";
    }
    return "";
  }
  ...
  function updateXlFileUi() {
    ...
    if (count) {
      count.textContent = hasFile ? `(${selectedXlFiles.length}/${XL_MAX_FILES})` : "";
    }
    if (list) {
    ...
    if (size) {
      const totalBytes = selectedXlFiles.reduce((sum, file) => sum + (file.size || 0), 0);
  ...
  function setSelectedXlFiles(fileList) {
    ...
    clearXlStatus();
    const uniqueFiles = files.filter(file => !selectedXlFiles.some(existing =>
  ...
  function resetXlUpload() {
    ...
    if (progressLabel) progressLabel.textContent = t('btn.uploading') || "上传中…";
  ...
  async function uploadXl(event) {
          ...
          while (!terminal) {
            ...
            for (const event of (eventData.events || [])) {
              if (event.type === "job_progress") {
                ...
                if (progressLabel) progressLabel.textContent = pending.length > 1
    ...
    const msg = d.warehouse_autosave
      ...
      : t('toast.upload_ok');
    toast(msg, "ok");
    const uploadedNames = (d.added || []).map(item => item.source_name).filter(Boolean);
  ...
  async function connectDB() {
    ...
    const r = await fetch(`/api/session/${state.SID}/connect-db`, {
      method: "POST", headers: window.BAA.auth.authHeaders({ "Content-Type": "application/json" }),
  ...
  async function loadSampleData() {
    ...
    try {
      ...
      const dims = [
        ...
        Array.isArray(previewMeta.columns) ? `${previewMeta.columns.length} 列` : "",
      ].filter(Boolean).join('，');
      const questionText = Array.isArray(d.suggested_questions) && d.suggested_questions.length
        ? `你可以直接试试：${d.suggested_questions.join(' / ')}`
        : "";

      toast(d.message || "示例数据已加载 ✓", "ok");
      sysMsg(`${d.source_name || '示例数据'}已连接${dims ? `（${dims}）` : ''}。${questionText}`);
  ...
  async function connectAPI() {
    ...
    if (!url && !hasSavedUrl) { errEl.textContent = t('api_err.no_url'); return; }
    errEl.textContent = "";
  ...
  export {
    ...
    resetXlUpload, chooseXlFile, closeXlUpload, removeXlFile, onXlFile, setupXlDropzone,
    uploadXl, removeSelectedXlFile, connectDB, connectGSheets, connectAPI, loadSampleData, toggleApiAuthValue,
  };
