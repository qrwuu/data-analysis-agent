...
def generate(
    ...
    **kwargs
    ...
    colors = get_colors_list(color_scheme, max(len(df) if df is not None else 10, 10))
    ...
    names_col = mapping.get("names") or mapping.get("name") or mapping.get("label") or names
    values_col = mapping.get("values") or mapping.get("value") or mapping.get("y") or values
    title = options.get("title", title)
    hole = float(options.get("hole", 0) or 0)
